...
class DynamicWeChat(_PluginBase):
    ...
    # 插件描述
    plugin_desc = "手机微信修改可信IP。验证码以？结尾发给企业微信应用。如：110301？"
    # 插件图标
    ...
    def init_plugin(self, config: dict = None):
        ...
        logger.info(f"当前公网 IP: {self._current_ip_address} playwright DEBUG")
        if config:
...
#
    def check_login_status(self, page):
        ...
        try:
            if self.find_qrc(page):
                logger.error(f"用户没有扫码/发送验证码或者登陆失败")
                self._ip_changed = False
                return False
        except Exception as e:
            # logger.error(f"检查登录状态时发生错误: {e}")
